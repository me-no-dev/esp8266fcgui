...
$(document).ready(function () {
    ...
    GUI.log('Running - OS: <strong>' + GUI.operating_system + '</strong>, ' +
        ...
        'Configurator: <strong>1.8.5 on ESP8266</strong>');

    $('#status-bar .version').text("1.8.5 on ESP8266");
    $('#logo .version').text("1.8.5");
    ...
    $('a', ui_tabs).click(function () {
        if ($(this).parent().hasClass('active') == false && !GUI.tab_switch_in_progress) { // only initialize when the tab isn't already active
            ...
            if (tabRequiresConnection && !CONFIGURATOR.connectionValid) {
                GUI.log(getLocalizedMessage('tabSwitchConnectionRequired'));
                return;
            }

            if (GUI.connect_lock) { // tab switching disabled while operation is in progress
                GUI.log(getLocalizedMessage('tabSwitchWaitForOperation'));
                return;
            }

            if (GUI.allowedTabs.indexOf(tab) < 0) {
                GUI.log(getLocalizedMessage('tabSwitchUpgradeRequired', [tabName]));
                return;
            }
