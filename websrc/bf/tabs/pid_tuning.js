...
TABS.pid_tuning.initialize = function (callback) {
    ...
    function pid_and_rc_to_form() {
        ...
        if (semver.gte(CONFIG.flightControllerVersion, "3.0.0")
            ...
            $('#pid-tuning .rate').text(getLocalizedMessage("pidTuningSuperRate"));
        } else {
            $('#pid-tuning .rate').text(getLocalizedMessage("pidTuningRate"));
        }
    ...
    function process_html() {
        ...
        function updatePidDisplay() {
            if (!self.showAllPids) {
                ...
                showAllButton.text(getLocalizedMessage("pidTuningShowAllPids"));
            } else {
                showAllPids();

                showAllButton.text(getLocalizedMessage("pidTuningHideUnusedPids"));
            }
        }
        ...
        $('#resetProfile').on('click', function(){
            ...
            MSP.promise(MSPCodes.MSP_SET_RESET_CURR_PID).then(function () {
                self.refresh(function () {
                    ...
                    GUI.log(getLocalizedMessage('pidTuningProfileReset'));
                });
            });
        });
        ...
        $('.tab-pid_tuning select[name="profile"]').change(function () {
            ...
            MSP.promise(MSPCodes.MSP_SELECT_SETTING, [self.currentProfile]).then(function () {
                self.refresh(function () {
                    ...
                    GUI.log(getLocalizedMessage('pidTuningLoadedProfile', [self.currentProfile + 1]));
                });
            });
        });
            ...
            $('.tab-pid_tuning select[name="rate_profile"]').change(function () {
                ...
                MSP.promise(MSPCodes.MSP_SELECT_SETTING, [self.currentRateProfile + self.RATE_PROFILE_MASK]).then(function () {
                    self.refresh(function () {
                        ...
                        GUI.log(getLocalizedMessage('pidTuningLoadedRateProfile', [self.currentRateProfile + 1]));
                    });
                });
            });
        ...
        if (semver.gte(CONFIG.apiVersion, CONFIGURATOR.pidControllerChangeMinApiVersion)) {
            ...
            GUI.log(getLocalizedMessage('pidTuningUpgradeFirmwareToChangePidController', [CONFIG.apiVersion, CONFIGURATOR.pidControllerChangeMinApiVersion]));
        ...
        $('a.refresh').click(function () {
            self.refresh(function () {
                GUI.log(getLocalizedMessage('pidTuningDataRefreshed'));
            });
        });
        ...
        $('a.update').click(function () {
            ...
            .then(function () {
                ...
                GUI.log(getLocalizedMessage('pidTuningEepromSaved'));
            });
        });
    ...
    if (GUI.active_tab === 'pid_tuning') {
        ...
        if (!self.updating && !self.dirty) {
            ...
            if (changedProfile || changedRateProfile) {
                self.refresh(function () {
                    if (changedProfile) {
                        GUI.log(getLocalizedMessage('pidTuningReceivedProfile', [CONFIG.profile + 1]));
                    }

                    if (changedRateProfile) {
                        GUI.log(getLocalizedMessage('pidTuningReceivedRateProfile', [CONFIG.rateProfile + 1]));
                    }
                });
            }
        }
