...
TABS.setup.initialize = function (callback) {
    ...
    function process_html() {
        ...
        if (semver.lt(CONFIG.apiVersion, CONFIGURATOR.backupRestoreMinApiVersionAccepted)) {
            ...
            GUI.log(getLocalizedMessage('initialSetupBackupAndRestoreApiVersion', [CONFIG.apiVersion, CONFIGURATOR.backupRestoreMinApiVersionAccepted]));
        }
        ...
        $('span.roll').text(getLocalizedMessage('initialSetupAttitude', [0]));
		// set pitch in interactive block
        $('span.pitch').text(getLocalizedMessage('initialSetupAttitude', [0]));
        // set heading in interactive block
        $('span.heading').text(getLocalizedMessage('initialSetupAttitude', [0]));
        ...
        $('a.calibrateAccel').click(function () {
            ...
            if (!self.hasClass('calibrating')) {
                ...
                MSP.send_message(MSPCodes.MSP_ACC_CALIBRATION, false, false, function () {
                    GUI.log(getLocalizedMessage('initialSetupAccelCalibStarted'));
                ...
                GUI.timeout_add('button_reset', function () {
                    ...
                    GUI.log(getLocalizedMessage('initialSetupAccelCalibEnded'));
        ...
        $('a.calibrateMag').click(function () {
            ...
            if (!self.hasClass('calibrating') && !self.hasClass('disabled')) {
                ...
                MSP.send_message(MSPCodes.MSP_MAG_CALIBRATION, false, false, function () {
                    GUI.log(getLocalizedMessage('initialSetupMagCalibStarted'));
                    $('#mag_calib_running').show();
                    $('#mag_calib_rest').hide();
                });

                GUI.timeout_add('button_reset', function () {
                    GUI.log(getLocalizedMessage('initialSetupMagCalibEnded'));
                    self.removeClass('calibrating');
        ...
        $('a.resetSettings').click(function () {
            MSP.send_message(MSPCodes.MSP_RESET_CONF, false, false, function () {
                GUI.log(getLocalizedMessage('initialSetupSettingsRestored'));
        ...
        $('div#interactive_block > a.reset').text(getLocalizedMessage('initialSetupButtonResetZaxisValue', [self.yaw_fix]));

        // reset yaw button hook
        $('div#interactive_block > a.reset').click(function () {
            self.yaw_fix = SENSOR_DATA.kinematics[2] * - 1.0;
            $(this).text(getLocalizedMessage('initialSetupButtonResetZaxisValue', [self.yaw_fix]));
        ...
        function get_slow_data() {
            ...
            MSP.send_message(MSPCodes.MSP_ANALOG, false, false, function () {
                bat_voltage_e.text(getLocalizedMessage('initialSetupBatteryValue', [ANALOG.voltage]));
                bat_mah_drawn_e.text(getLocalizedMessage('initialSetupBatteryMahValue', [ANALOG.mAhdrawn]));
                bat_mah_drawing_e.text(getLocalizedMessage('initialSetupBatteryAValue', [ANALOG.amperage.toFixed(2)]));
                rssi_e.text(getLocalizedMessage('initialSetupRSSIValue', [((ANALOG.rssi / 1023) * 100).toFixed(0)]));
            });

            if (have_sensor(CONFIG.activeSensors, 'gps')) {
                MSP.send_message(MSPCodes.MSP_RAW_GPS, false, false, function () {
                    gpsFix_e.html((GPS_DATA.fix) ? getLocalizedMessage('gpsFixTrue') : getLocalizedMessage('gpsFixFalse'));
        ...
        function get_fast_data() {
            MSP.send_message(MSPCodes.MSP_ATTITUDE, false, false, function () {
	            roll_e.text(getLocalizedMessage('initialSetupAttitude', [SENSOR_DATA.kinematics[0]]));
	            pitch_e.text(getLocalizedMessage('initialSetupAttitude', [SENSOR_DATA.kinematics[1]]));
                heading_e.text(getLocalizedMessage('initialSetupAttitude', [SENSOR_DATA.kinematics[2]]));
...
TABS.setup.initializeInstruments = function() {
    var options = {size:90, showBox : false, img_directory: 'images/ind/'};
